...
import { IndicatorItem } from 'components/cards/indicators-post';
...
export function AllIndicators() {
  ...
  const indicadoresSemSelic = indicadores.filter(n => n.mn_codigo !== 242)
      ...
      <Container>
        <ScrollAnimation animateIn="slideInRight" animateOut="fadeOut">
          <Slider sx={styles.grid} {...settings}>
            {indicadoresSelic?.map((post) => (
              <IndicatorItem key={post.mn_codigo} post={post} />
            ))}

            {indicadoresSemSelic?.map((post) => (
              <IndicatorItem key={post.mn_codigo} post={post} />
            ))}
          </Slider>
        </ScrollAnimation>
